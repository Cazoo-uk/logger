...
test('When logging an error', async ({ match, is }) => {
  const stream = sink()
...
test('When logging an error with a custom message', async ({ match, is }) => {
  const stream = sink()
  const error = new Error('A thing you wish had worked did not, in fact, work')
...
test('When logging an error as a warning', async ({ match, is }) => {
  const stream = sink()
  const error = new Error('A thing you wish had worked did not, in fact, work')
